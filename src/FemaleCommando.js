...
export class FemaleCommando extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    super(scene, x, y, "female_commando_idle_frame1")
    ...
    this.hurtingDuration = gameConfig.playerConfig.hurtingDuration.value
    this.invulnerableTime = gameConfig.playerConfig.invulnerableTime.value
    ...
    this.collisionBoxWidth = 367 * 0.9
    ...
    // Play idle animation
    this.play("female_commando_idle_anim")
  ...
  // Initialize all sounds
  initializeSounds() {
    this.rifleShotSound = this.scene.sound.add("rifle_shot", { volume: 0.3 })
  }
  ...
  createAnimations() {
    ...
    // Idle animation
    if (!anims.exists("female_commando_idle_anim")) {
      anims.create({
        key: "female_commando_idle_anim",
        frames: [
          {
            key: "female_commando_idle_frame1",
            duration: 800,
          },
          {
            key: "female_commando_idle_frame2",
            duration: 800,
          },
        ],
        repeat: -1,
    ...
    // Walk animation
    if (!anims.exists("female_commando_walk_anim")) {
      anims.create({
        key: "female_commando_walk_anim",
        frames: [
          {
            key: "female_commando_walk_frame1",
            duration: 300,
          },
          {
            key: "female_commando_walk_frame2",
            duration: 300,
          },
          {
            key: "female_commando_walk_frame3",
            duration: 300,
          },
          {
            key: "female_commando_walk_frame4",
            duration: 300,
          },
        ],
        repeat: -1,
    ...
    // Jump Up animation
    if (!anims.exists("female_commando_jump_up_anim")) {
      anims.create({
        key: "female_commando_jump_up_anim",
        frames: [
          {
            key: "female_commando_jump_frame1",
            duration: 200,
          },
        ],
        repeat: 0,
    ...
    // Jump Down animation
    if (!anims.exists("female_commando_jump_down_anim")) {
      anims.create({
        key: "female_commando_jump_down_anim",
        frames: [
          {
            key: "female_commando_jump_frame2",
            duration: 300,
          },
        ],
        repeat: 0,
    ...
    // Attack animation
    if (!anims.exists("female_commando_attack_anim")) {
      anims.create({
        key: "female_commando_attack_anim",
        frames: [
          {
            key: "female_commando_attack_frame1",
            duration: 100,
          },
          {
            key: "female_commando_attack_frame2",
            duration: 200,
          },
        ],
        repeat: 0,
    ...
    // Die animation
    if (!anims.exists("female_commando_die_anim")) {
      anims.create({
        key: "female_commando_die_anim",
        frames: [
          {
            key: "female_commando_die_frame1",
            duration: 500,
          },
          {
            key: "female_commando_die_frame2",
            duration: 1000,
          },
        ],
        repeat: 0,
    ...
    // Crouch animation
    if (!anims.exists("female_commando_crouch_anim")) {
      anims.create({
        key: "female_commando_crouch_anim",
        frames: [
          {
            key: "female_commando_crouch_frame1",
            duration: 300,
          },
          {
            key: "female_commando_crouch_frame2",
            duration: 400,
          },
        ],
        repeat: 0,
  ...
  handleDying() {
    if (this.health <= 0 && !this.isDead) {
      ...
      this.play("female_commando_die_anim", true)
      this.resetOriginAndOffset()
      this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation, frame) => {
        if (animation.key === "female_commando_die_anim") {
  ...
  handleCrouching(crouchKey) {
    if (Phaser.Input.Keyboard.JustDown(crouchKey) && !this.isCrouching) {
      ...
      this.play("female_commando_crouch_anim", true)
      this.resetOriginAndOffset()
      
      this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation, frame) => {
        if (animation.key === "female_commando_crouch_anim") {
  ...
  handleAttacks(shootKey, meleeKey) {
    ...
    if (Phaser.Input.Keyboard.JustDown(shootKey) && !this.isAttacking) {
      ...
      this.play("female_commando_attack_anim", true)
      this.resetOriginAndOffset()
      ...
      this.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (animation, frame) => {
        if (animation.key === "female_commando_attack_anim") {
          this.isAttacking = false
        }
      })
    }
  ...
  handleMovement(cursors) {
    ...
    if (upPressed && this.body.onFloor()) {
    ...
    if (!this.body.onFloor()) {
      if (this.body.velocity.y < 0) {
        this.play("female_commando_jump_up_anim", true)
        this.resetOriginAndOffset()
      } else {
        this.play("female_commando_jump_down_anim", true)
        this.resetOriginAndOffset()
      }
    } else if (Math.abs(this.body.velocity.x) > 0) {
      this.play("female_commando_walk_anim", true)
      this.resetOriginAndOffset()
    } else {
      this.play("female_commando_idle_anim", true)
      this.resetOriginAndOffset()
    }
  }
  ...
  resetOriginAndOffset() {
    ...
    if (currentAnim) {
      switch(currentAnim.key) {
        case "female_commando_idle_anim":
          baseOriginX = 0.5;
          baseOriginY = 1.0;
          break;
        case "female_commando_walk_anim":
          baseOriginX = 0.488;
          baseOriginY = 1.0;
          break;
        case "female_commando_jump_up_anim":
        case "female_commando_jump_down_anim":
          baseOriginX = 0.389;
          baseOriginY = 1.0;
          break;
        case "female_commando_attack_anim":
          baseOriginX = 0.261;
          baseOriginY = 1.0;
          break;
        case "female_commando_die_anim":
          baseOriginX = 0.615;
          baseOriginY = 1.0;
          break;
        case "female_commando_crouch_anim":
          baseOriginX = 0.358;
          baseOriginY = 1.0;
          break;
        default:
